...
export default function BookPage() {
  ...
  async function handleRedeemBooking() {
    ...
    try {
      const { error: dbError } = await supabase.from('bookings').insert({ name: redeemBooking.name, email: redeemBooking.email, phone: redeemBooking.phone, service_type: redeemedVoucher.session_type, people_count: redeemedVoucher.session_duration === 60 ? 3 : 1, session_duration: redeemedVoucher.session_duration, session_price: 0, slot_date: redeemBooking.date, slot_time: redeemBooking.time, voucher_code: redeemedVoucher.code, notes: redeemBooking.notes, status: 'confirmed' });
      if (dbError) throw dbError;
      await supabase.from('vouchers').update({ status: 'used', redeemed_at: new Date().toISOString() }).eq('id', redeemedVoucher.id);
