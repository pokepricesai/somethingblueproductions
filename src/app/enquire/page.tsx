...
export default function EnquirePage() {
  ...
  const handleSubmit = async (e: React.MouseEvent) => {
    ...
    else { setStatus('success'); setForm({ name: '', email: '', phone: '', service: '', date: '', message: '' }); }
  };
  ...
  return (
    <>
      <style>{`
        .enq-pad { padding: 3rem 1.5rem; }
        .enq-hero { padding: 8rem 1.5rem 4rem; }
        .enq-layout { display: grid; grid-template-columns: 1fr; gap: 3rem; max-width: 1100px; margin: 0 auto; }
        .enq-name-row { display: grid; grid-template-columns: 1fr; gap: 1rem; }
        .enq-phone-row { display: grid; grid-template-columns: 1fr; gap: 1rem; }

        @media (min-width: 640px) {
          .enq-name-row { grid-template-columns: 1fr 1fr; }
          .enq-phone-row { grid-template-columns: 1fr 1fr; }
        }

        @media (min-width: 900px) {
          .enq-pad { padding: 4rem 4rem; }
          .enq-hero { padding: 10rem 4rem 5rem; }
          .enq-layout { grid-template-columns: 1fr 380px; }
        }
      `}</style>

      {/* ─── HERO ─── */}
      <section className="enq-hero" style={{ backgroundColor: '#0d1b2a' }}>
        <div style={{ maxWidth: '600px' }}>
          ...
          <h1 style={{ fontFamily: "'Carose', sans-serif", fontWeight: 300, fontSize: 'clamp(1.8rem, 4vw, 3rem)', color: '#E8DDB5', lineHeight: 1.2, textTransform: 'none', marginBottom: '1.2rem' }}>
      ...
      {/* ─── FORM + SIDEBAR ─── */}
      <section className="enq-pad" style={{ backgroundColor: '#F5F0E8' }}>
        <div className="enq-layout">

          {/* Form */}
          <div>
            {status === 'success' ? (
              <div style={{ padding: '3rem 2rem', backgroundColor: '#E8DDB5', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div className="enq-name-row">
                  <div>
                    ...
                    <input name="name" value={form.name} onChange={handleChange} placeholder="Sarah & Tom" style={inputStyle}
                      onFocus={e => (e.currentTarget.style.borderColor = '#1B3A5C')}
                      onBlur={e => (e.currentTarget.style.borderColor = '#DDD5C0')} />
                  </div>
                  <div>
                    <label style={labelStyle}>Email address *</label>
                    <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="hello@example.com" style={inputStyle}
                      onFocus={e => (e.currentTarget.style.borderColor = '#1B3A5C')}
                      onBlur={e => (e.currentTarget.style.borderColor = '#DDD5C0')} />
                  </div>
                </div>

                <div className="enq-phone-row">
                  <div>
                ...
                <div>
                  <label style={labelStyle}>When are you thinking?</label>
                  <input name="date" value={form.date} onChange={handleChange} placeholder="e.g. June 2025, or flexible" style={inputStyle}
                    onFocus={e => (e.currentTarget.style.borderColor = '#1B3A5C')}
                    onBlur={e => (e.currentTarget.style.borderColor = '#DDD5C0')} />
                </div>
                ...
                <div>
                  ...
                  <textarea name="message" value={form.message} onChange={handleChange}
                    placeholder="Anything you'd like us to know — your venue, location, style preferences, or just a general hello."
                ...
                <div>
                  <button onClick={handleSubmit} disabled={status === 'sending'}
                    style={{ fontFamily: "'Carose', sans-serif", fontSize: '0.68rem', letterSpacing: '0.2em', textTransform: 'uppercase', backgroundColor: status === 'sending' ? '#9E9282' : '#1B3A5C', color: '#F5F0E8', border: 'none', padding: '1.1rem 3rem', cursor: status === 'sending' ? 'not-allowed' : 'pointer', width: '100%' }}
