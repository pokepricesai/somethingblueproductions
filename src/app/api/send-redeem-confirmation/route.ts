...
export async function POST(req: NextRequest) {
  try {
    const { name, email, phone, date, time, notes, voucher } = await req.json();

    const sessionLabel = voucher.session_duration === 60 ? 'Family Session' : 'Studio Session';
    const imagesLabel = voucher.session_duration === 60 ? '10–20' : '5–10';
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:'Georgia',serif;">
  <div style="max-width:600px;margin:0 auto;background:#ffffff;">
    ...
    <div style="padding:40px;">
      <p style="font-family:Georgia,serif;font-size:16px;color:#2C2820;line-height:1.7;margin:0 0 24px;">Hi ${name},</p>
      ...
      <div style="background:#F5F0E8;border-left:3px solid #1B3A5C;padding:24px;margin-bottom:32px;">
        <table style="width:100%;border-collapse:collapse;">
          ...
          <tr><td style="font-family:Georgia,serif;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#9E9282;padding:6px 0;">Duration</td><td style="font-family:Georgia,serif;font-size:15px;color:#1B3A5C;padding:6px 0;">${voucher.session_duration} minutes</td></tr>
          ...
          <tr><td style="font-family:Georgia,serif;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#9E9282;padding:6px 0;">Voucher</td><td style="font-family:Georgia,serif;font-size:15px;color:#1B3A5C;padding:6px 0;">${voucher.code} · Gift from ${voucher.buyer_name}</td></tr>
          <tr><td style="font-family:Georgia,serif;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#9E9282;padding:6px 0;">Paid</td><td style="font-family:Georgia,serif;font-size:15px;color:#16a34a;padding:6px 0;">£0 — covered by gift voucher</td></tr>
        </table>
      </div>
...
<body style="margin:0;padding:20px;background:#F5F0E8;font-family:Georgia,serif;">
  <div style="max-width:500px;margin:0 auto;background:#fff;padding:32px;border:1px solid #DDD5C0;">
    <p style="font-size:11px;letter-spacing:3px;text-transform:uppercase;color:#9E9282;margin:0 0 16px;">Voucher Redeemed</p>
    ...
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tr><td style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#9E9282;padding:8px 0;border-bottom:1px solid #F5F0E8;">Phone</td><td style="font-size:14px;color:#2C2820;padding:8px 0;border-bottom:1px solid #F5F0E8;"><a href="tel:${phone}" style="color:#1B3A5C;">${phone}</a></td></tr>
      <tr><td style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#9E9282;padding:8px 0;border-bottom:1px solid #F5F0E8;">Voucher</td><td style="font-size:14px;color:#2C2820;padding:8px 0;border-bottom:1px solid #F5F0E8;">${voucher.code}</td></tr>
      <tr><td style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#9E9282;padding:8px 0;border-bottom:1px solid #F5F0E8;">Gift from</td><td style="font-size:14px;color:#2C2820;padding:8px 0;border-bottom:1px solid #F5F0E8;">${voucher.buyer_name}</td></tr>
      <tr><td style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#9E9282;padding:8px 0;">Notes</td><td style="font-size:14px;color:#2C2820;padding:8px 0;">${notes || 'None'}</td></tr>
    </table>
    ...
    // Send confirmation to the person redeeming
    await sendEmail(
      [{ email, name }],
    ...
    await sendEmail(
      ...
      `Voucher redeemed — ${name} · ${dateFormatted} ${timeFormatted}`,
      adminHtml,
    ...
    console.error('Redeem confirmation email error:', error);
